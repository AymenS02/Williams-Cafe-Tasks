...
export default function HomePage() {
  ...
  async function handleSubmit(e, taskId) {
    e.preventDefault();
    const formData = new FormData(e.target);
    const initials = formData.get("initials");
    const photos = formData.getAll("photos").map((f) => URL.createObjectURL(f));
            ...
            >
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1">
                  <h2 className="text-2xl font-bold text-amber-900 mb-1" style={{ fontFamily: 'Georgia, serif' }}>
                    {task.title}
                  </h2>
              ...
              {task.status === "COMPLETED" ? (
                <div className="bg-green-100 border-2 border-green-500 text-green-800 p-4 rounded-xl mt-4">
                  <span className="text-lg">
                    ✅ Completed by <strong className="font-bold">{task.initials}</strong>
                  </span>
                </div>
              ) : (
                <form
              ...
              {task.photos && task.photos.length > 0 && (
                <button
                  ...
                  className="mt-4 bg-amber-700 text-amber-50 px-4 py-2 rounded-lg hover:bg-amber-600 transition-colors shadow-md font-medium"
                >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
          <div className="bg-amber-50 rounded-2xl max-w-4xl w-full p-6 relative overflow-y-auto max-h-full border-4 border-amber-900 shadow-2xl">
            ...
            <h3 className="text-2xl font-bold text-amber-900 mb-4" style={{ fontFamily: 'Georgia, serif' }}>
              Task Photos
            </h3>
